import React from 'react'
import { NavLink } from 'react-router'

function Header() {
  return (
    <div className='flex justify-between bg-gray-600 '>
      <img  className="rounded-56xl"width='80px' src="data:image/png;base64," alt="" />
      <nav className='pl-10'>
        <ul className='flex justify-between w-100 m-5'>
          <li>
            <NavLink to="/" className={({isActive})=>isActive?"text-black-500 bg-blue-700 p-1.5":""}>Home</NavLink>
          </li>
          <li>
            <NavLink to="AddUser"  className={({isActive})=>isActive?"text-black-600  bg-blue-700 p-1.5":""}>AddUser</NavLink>
          </li>
          <li>
            <NavLink to="UsersList"  className={({isActive})=>isActive?"text-black-600  bg-blue-700 p-1.5":""}>UsersList</NavLink>
          </li>
          
        </ul>
      </nav>
    </div>
  )
}

export default Header;